...
theme(
    ...
    legend.text = element_text(size = 20)  # Adjust the size value as needed
  ) 
...
Let's try to play with IPUs
```{r IPU}

# load the dataset with distance matrix

library(readr)

matrix_path <- file.path(dataset_folder, "/distance_matrix.csv")
data <- read_csv(matrix_path)

# the rownames didn't save so we need to do that again

# Set row names to be the column names
rownames(data) <- names(data)

# load the IPU df

IPU_path <- file.path(dataset_folder, "/IPU_data.csv")
IPU_data <- read_csv(IPU_path)


# also load the df with lots of info
df_path <- file.path(dataset_folder, "/meanDist_pertrial.csv")
df <- read_csv(df_path)


# merge df with IPU_data

df_IPU <- merge(df, IPU_data, by = "ID", all.x = TRUE)

```


Let's plot number of IPUs
```{r}

ipu_count <- IPU_distance %>%
  group_by(participant, trial) %>%
  summarize(Unique_IPU_count = n_distinct(IPU_count))

# merge df with IPU_data

df_IPU <- merge(df_IPU, ipu_count, by = c("participant", "trial"), all.x = TRUE)


ipu_trial_plot <- ggplot(df_IPU, aes(x = word, y = Unique_IPU_count)) +
  geom_point(size = 2, color = viridis(1, option = "A"), position = position_dodge(width = 0.5)) +
  geom_boxplot(width = 0.2, fill = viridis(1), alpha = 0.5, position = position_dodge(width = 0.5)) +
  labs(title = "",
       x = "Concept", y = "Number of IPUs per trial", size= 2) +
  theme_minimal() +
theme(
    plot.title = element_text(hjust = 0.5),
    legend.position = "none",
...
# Define the file path and name for saving the plot
plot_filename <- file.path(plots_folder, "IPU_per_trial.png")  # You can choose the file format (e.g., PNG, PDF, etc.)

# Save the plot in high resolution to the specified folder and filename
ggsave(plot_filename, ipu_trial_plot, width = 8, height = 6, dpi = 300) 


```
How many segments is in IPU
```{r}

ipu_segment_count <- IPU_distance %>%
  group_by(ID_IPU) %>%
  summarize(Unique_IPU_segment_count = n_distinct(segmentCount))

# merge df with IPU_data

df_IPU <- merge(df_IPU, ipu_segment_count, by = "ID_IPU", all.x = TRUE)

ipu_plot <- ggplot(df_IPU, aes(x = word, y = Unique_IPU_segment_count)) +
  geom_point(size = 1, color = viridis(1, option = "A"), position = position_dodge(width = 0.5)) +
  geom_boxplot(width = 0.2, fill = viridis(1), alpha = 0.5, position = position_dodge(width = 0.5)) +
  labs(title = "",
       x = "Concept", y = "Number of sounds between pauses", size= 20) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5),
    legend.position = "none",
...
# Define the file path and name for saving the plot
plot_filename <- file.path(plots_folder, "sounds_per_IPU.png")  # You can choose the file format (e.g., PNG, PDF, etc.)

# Save the plot in high resolution to the specified folder and filename
ggsave(plot_filename, ipu_plot, width = 8, height = 6, dpi = 300) 

```


Let's get the distance measures for IPUs
```{r}
...
for (i in 1:nrow(IDs_list)) {
  ...
  # Select the relevant rows and columns in the distance matrix
  subset_matrix <- data[id_list, id_list]
...
# merge the original df with avg distance
IPU_distance <- merge(df_IPU, IDs_list, by.x = "ID_IPU", by.y = "ID_IPU")
...
ggplot(IPU_distance, aes(x = reorder(category, -Dist_mean.y), y = Dist_mean.y)) +
  geom_point(size = 2, color = viridis(1, option = "A"), position = position_dodge(width = 0.5)) +
  geom_boxplot(width = 0.2, fill = viridis(1), alpha = 0.5, position = position_dodge(width = 0.5)) +
  labs(title = "Average Distance Between IPUs Across Categories",
       x = "Category", y = "Average Distance per IPU") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5), legend.position = "none")
